let express = require('express'),
  ...
  chai = require('chai'),
  assert = chai.assert,
  Skylith = require('../src/OpenIDProvider'),
  endpoint = 'http://localhost:3030/openid', // This doesn't have to be real!
  skylith = new Skylith({providerEndpoint: endpoint,
...
exports = module.exports = {
  ...
  isDelegated,
  identity (name) {
    return endpoint + '?u=' + encodeURIComponent(name);
  }
};
...
function calculateCommType (res, requestMode) {
  let commType = COMM_TYPE[requestMode];
...
function checkAuthDelegate () {
  currentCheckAuth(...arguments);
}

function checkAuth (options) {
  if (typeof currentCheckAuth !== 'undefined') {
  ...
  let error = 'checkAuth wasn\'t called';
...
function standardExpectations (reqParams) {
  return function (res) {
    ...
    let commType = calculateCommType(res, reqParams['openid.mode']),
      resParams;
    ...
    let responseMode = resParams['openid.mode'];
...
var expectationsByMode = {
  id_res (req, res, reqParams, resParams) {
    ...
    let nonce = resParams['openid.response_nonce'],
  ...
  cancel (req, res, reqParams, resParams) {},
  setup_needed (req, res, reqParams, resParams) {},
  error (req, res, reqParams, resParams) {}
};
