...
var DEFAULT_ASSOCIATION_EXPIRY = 30; // TODO
...
function OpenIDProvider(options) {
    ...
    this.sendHtmlDiscoveryResponse = function(req, res, user, next) {
        res.type('text/html');
    ...
    function unencryptedAssociation(request, res, next) {
        createMac(request.assoc_type, function(err, macBuffer, hashAlgorithm) {
            ...
            var association = new Association(hashAlgorithm, macBuffer.toString('base64'), DEFAULT_ASSOCIATION_EXPIRY, false);
    ...
    function checkIdSetupComplete(req, res, username, next) {
        var request = req.signedCookies[COOKIE],
            association,
            nonce = new Date().toISOString().slice(0, -5) + 'Z' + crypto.randomBytes(4).toString('hex'),
            response = {
                mode: 'id_res',
        ...
        function privateAssociation() {
            if (!association) {
                ...
                createMac('HMAC-SHA256', function(err, macBuffer, hashAlgorithm) {
                    ...
                    association = new Association(hashAlgorithm, macBuffer.toString('base64'), DEFAULT_ASSOCIATION_EXPIRY, true);
