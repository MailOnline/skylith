...
function OpenIDProvider(options) {
    ...
    this.sendDiscoveryResponse = function(req, res, identity, next) {
...
console.log('>>>',options,providerEndpoint)
            if (identity) {
            ...
            if (identity) {
                res.end(util.format(HTML_VALIDATION_RESPONSE_TEMPLATE, providerEndpoint, identity));
            } else {
    ...
    this.completeAuth = function(req, res, authResponse, next) {
        checkIdSetupComplete(req, res, authResponse, next);
    }

    this.cancelAuth = function(req, res, next) {
        checkIdSetupCancel(req, res, next);
    }
    ...
    function checkIdSetup(request, req, res, message) {
        ...
        if (!req.session.openid) req.session.openid = {};
        req.session.openid.request = request;
        ...
        if (axRequest) {
            ...
            req.session.openid.ax = axRequest;
        }

        res.redirect(302, loginUrl);
    }

    function checkIdSetupCancel(req, res, next) {
        var request = req.session && req.session.openid && req.session.openid.request;

        if (!request) return res.send(400);

        delete req.session.openid.request;
        delete req.session.openid.ax;
    ...
    function checkIdSetupComplete(req, res, authResponse, next) {
        var request = req.session && req.session.openid && req.session.openid.request,
            association,
            nonce = {
        ...
        var response = {
                ...
                claimed_id: authResponse.identity,
                identity: authResponse.identity,
                return_to: request.return_to,
                response_nonce: nonce.id
            };

        if (req.session.openid.ax) {
            var axResponse = authResponse.ax,
                aliasNum = 1;

            // TODO hardcoding 'ax' as the namespace might be a bad idea
            response['ns.ax'] = 'http://openid.net/srv/ax/1.0';
            response['ax.mode'] = 'fetch_response';

            for (var type in axResponse) {
                var alias = 'alias' + aliasNum++,
                    value = axResponse[type];

                response['ax.type.' + alias] = type;

                if (util.isArray(value)) {
                    response['ax.count.' + alias] = value.length;
                    for (var i = 0; i < value.length; i++) {
                        response['ax.value.' + alias + '.' + (i + 1)] = value[i];
                    }
                } else {
