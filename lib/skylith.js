...
function OpenIDProvider(options) {
    ...
    this.sendDiscoveryResponse = function(req, res, user, next) {
        ...
        if (req.accepts('application/xrds+xml')) {
            ...
            if (user) {
                res.end(util.format(HTML_VALIDATION_RESPONSE_TEMPLATE, this.providerEndpoint(), localId(user)));  // TODO: this mapping should be the responsibility of the provider
            } else {
